<template>
    <div class="question">
        ...
        <template v-if="operation==='viewAnswer' || isShowPractiseResult" style="margin-left: 16px">
            <span v-if="answer===detail.answer" style="color: green">【正确】</span>
            <span v-else style="color: red">【错误】正确答案：{{detail.answer}}</span>
        </template>
        <Input v-model.trim="answer" :placeholder="placeholder" @on-blur="onBlur" :disabled="operation==='viewAnswer'"
               v-if="operation==='answer' || operation==='viewAnswer' || operation==='practise'"/>
    </div>
</template>
<script>
    ...
    export default {
        ...
        props: {
            ...
            detail: {type: Object},
            isShowPractiseResult: {type: Boolean, default: false}
        },
        data() {
            return {
                placeholder: '',
