<template>
  <div class="bg">
    <div class="outer">
      ...
      <div class="inner">
        <Form ref="basic" :model="basic"  :rules="ruleValidate" :label-width="120">
          ...
          <FormItem v-if="roleId == '0'" label="账户名称" prop="companyName"  >
    ...
    <div class="outer" v-if="roleId == '1'">
      <p>接口信息</p>
      <div class="inner">

        <Form ref="more" :model="more" :label-width="120">

          <FormItem label="签名审核回调地址" prop="accountNumber">
            <Row>
              <Col span="21">
                <Input v-model="basic.signatureCallbackUrl" disabled></Input>
              </Col>
          ...
          <FormItem label="模板审核回调地址" prop="accountNumber">
            <Row>
              <Col span="21">
                <Input v-model="basic.tmplAuditCallbackUrl" disabled></Input>
              </Col>
              <Col span="2" style="padding-left: 24px">
              </Col>
            </Row>
          ...
          <FormItem label="API_KEY" prop="accountNumber">
            <Row>
              <Col span="21">
                <Input v-model="apiKey" disabled></Input>
              </Col>
              <Col span="2" style="padding-left: 24px">
              </Col>
            </Row>
          ...
          <FormItem label="API_secret" prop="accountNumber">
            <Row>
              <Col span="21">
                <Input v-model="apiSecret" disabled></Input>
              </Col>
              <Col span="2" style="padding-left: 24px">
                <Button type="primary" @click="resetApiSecret()">重置</Button>
              </Col>
            </Row>
          </FormItem>
        </Form>
  ...
  export default {
    ...
    methods:{
      ...
      setBasicInfo(){
        ...
        this.basic.signatureCallbackUrl = this.signatureCallbackUrl;
        this.basic.tmplAuditCallbackUrl = this.tmplAuditCallbackUrl;
      },
      ...
      resetApiSecret(){
        this.handleResetApiSecret({accountId:this.accountId}).then(
          res =>{
      ...
      handleSubmit(name){
        if(name=='basic'){
          this.handleUpdateNickName({accountId:this.accountId, nickname:this.basic.nickname}).then(
            res =>{
              this.updateNickName({accountNickname:this.basic.nickname});
    ...
    computed: {
      ...mapState({
        ...
        signatureCallbackUrl: state => state.account.signatureCallbackUrl,
        tmplAuditCallbackUrl: state => state.account.tmplAuditCallbackUrl,
        ...
        apiKey:state => state.account.apiKey,
        apiSecret:state => state.account.apiSecret,
      }),
