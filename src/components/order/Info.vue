  ...
  export default {
    ...
    computed: {
      ...
      columns() {
        const columns = [
          {
            title: '订单号', key: 'number', align: 'center', ellipsis: true, minWidth: 80,
            render: (h, params) => showTip(h, params.row.number)
          },
          {
            title: '订单总额（元）', key: 'price', align: 'center', ellipsis: true, minWidth: 80,
            render: (h, params) => showTip(h, params.row.price)
          },
          {
            title: '下单时间', key: 'createTs', align: 'center', ellipsis: true, minWidth: 80,
            render: (h, params) => showTip(h, params.row.createTs)
          },
          {
            title: '订单状态', key: 'status', align: 'center', ellipsis: true, minWidth: 80,
            render: (h, params) => {
              const {status} = params.row;
              let text = status === 1 ? '待支付' : status === 2 ? '已支付' : status === 3 ? '已完成' : '已失效';
              let color = status === 1 ? 'black' : status === 2 ? 'blue' : status === 3 ? 'green' : 'gray';
              return showTip(h, text, color);
            }
          },
          {
            title: '收货地址', key: 'address', align: 'center', ellipsis: true, minWidth: 80,
            render: (h, params) => showTip(h, params.row.address)
          },
          // {
